...
class Projects extends MY_Controller {
    ...
    function assign($id = FALSE)
    {
        ...
        if($_POST){
            unset($_POST['send']);
            $id = $_POST['id'];
            $sql = "DELETE FROM project_has_workers WHERE project_id=".$id;
            $query = $this->db->query($sql);
            foreach ($_POST["user_id"] as &$value):
                ...
                send_notification($receiver->email, $this->lang->line('application_notification_project_assign_subject'), $this->lang->line('application_notification_project_assign'));
            endforeach;
